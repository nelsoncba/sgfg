...
class ProductoController extends Controller
{
    ...
    public function actionCreate()
    {
        $request = Yii::$app->request;
        $model = new Producto(); 
        $producto = Producto::find()->orderBy(['id'=>SORT_DESC])->one();
        
        if(!isset($producto)){
            $model->codigo = '000001';
        }else{
         $codigo = $producto->codigo + 1;
         $model->codigo = str_pad((string)$codigo, 6, "0", STR_PAD_LEFT); 
        }
            ...
            */
            ...
            if($request->isGet){
                ...
                return [
                    ...
                    'title'=> "Nuevo Producto",
                    'content'=>'<span class="text-success">Producto creado correctamente</span>',
                    'footer'=> Html::button('Cerrar',['class'=>'btn btn-default pull-left','data-dismiss'=>"modal"]).
